...
JSONL = PROCESS / 'pt_unified_full_20260611.jsonl'
TASK_CACHE = PROCESS / 'pt_unified_full_20260611_tasks.json'
...
def google_translate(en):
    ...
    for attempt in range(GOOGLE_RETRIES):
        try:
            wait_google_slot()
            tr = GoogleTranslator(source='en', target='pt').translate(safe)
...
def dify_translate(en):
    ...
    payload = {
        ...
        'user': 'pt-unified-google-first-20260611',
    }
    last_error = None
...
def iter_rows(path):
    ...
    en_i = lower.index('en')
    ptpt_i = lower.index('pt-pt')
    ptbr_i = lower.index('pt-br')
    for row in rows:
        key = str(row[key_i] or '').strip()
        if not key:
            continue
        yield key, str(row[en_i] or '').strip(), str(row[ptpt_i] or '').strip(), str(row[ptbr_i] or '').strip()
...
def load_jsonl_done():
    ...
    with open(JSONL, encoding='utf-8') as f:
        for line in f:
            ...
            # Direct Dify records from the interrupted run did not follow
            # Google-first policy, so retry them instead of treating as done.
            if (
                rec.get('status') == 'ok'
                and rec.get('engine') != 'dify'
                and en
                and qc_ok(en, tr)
            ):
                done[en] = tr
...
def main():
    ...
    # Blocked records are retried in this Google-first run; only successful JSONL entries are skipped.

#!/usr/bin/env python3
"""基于最新全量缺失任务进行补翻，并按语种输出上传格式 Excel。"""
...
OUTPUT_DIR = BASE / "05_上传格式" / "20260713_补翻"
UPLOADABLE_DIR = BASE / "05_上传格式" / "20260713_补翻_可上传"
JSONL = PROCESS / "refill_translations_20260713.jsonl"
QC_REPORT = PROCESS / "refill_translation_qc_20260713.json"
LEGACY_JSONL_FILES = [PROCESS / "retry_translations_20260624.jsonl"]
RUN_DATE = "20260713"
MAX_UPLOAD_ROWS = 25000
SKIP_TRANSLATE_LANGS = {"nb"}
...
BCP47_TO_LANG.update({value.lower(): key for key, value in LANG_TO_BCP47.items()})
GOOGLE_LANG = {
    "ar": "ar",
    "de": "de",
    "el-gr": "el",
...
def load_tasks(langs=None):
    ...
    for path in sorted(TASK_DIR.glob(f"*_refill_tasks_{RUN_DATE}.json")):
        lang = path.name.replace(f"_refill_tasks_{RUN_DATE}.json", "")
...
def hydrate_done_from_legacy_jsonl(tasks_by_lang, done):
    ...
    appended = 0
    by_en = defaultdict(dict)
    for path in LEGACY_JSONL_FILES:
        if not path.exists():
            continue
        hit = 0
        with open(path, encoding="utf-8") as f:
            for line in f:
                ...
                if record.get("status") != "ok":
                    continue
                en = record.get("en") or ""
                key = record.get("en_hash") or md5(en)
                if key not in needed:
                    continue
                translations = record.get("translations") or {}
                for lang in sorted(needed[key]):
                    ...
                    if not translation:
                        continue
                    source_en = en_by_hash[key]
                    if lang == "sr":
                        translation = latinize_sr(translation)
                    if qc_issues(source_en, translation, lang):
                        continue
                    if key not in done:
                        done[key] = {"en_hash": key, "en": source_en, "translations": {}, "status": "ok"}
                    done[key]["translations"][lang] = translation
                    by_en[source_en][lang] = translation
                    hit += 1
        if hit:
            log(f"旧 JSONL 缓存命中 {path.name}: {hit} 个语种单元格")
    ...
    for en, translations in by_en.items():
        append_jsonl({
            ...
            "missing_langs": [],
            "status": "ok",
            "engine": "legacy_retry_jsonl",
            "ts": datetime.now(timezone.utc).isoformat(),
        })
        appended += 1
    if appended:
        log(f"旧 JSONL 缓存写入 JSONL 唯一 EN 数：{appended}")
    return done
        ...
        for attempt in range(4):
            try:
                ...
                for item in output:
                    ...
                    if not code:
                        continue
                    lang = BCP47_TO_LANG.get(code) or BCP47_TO_LANG.get(str(code).lower())
                    if not lang:
                        continue
...
def chunked(rows):
    for start in range(0, len(rows), MAX_UPLOAD_ROWS):
        yield start // MAX_UPLOAD_ROWS + 1, rows[start:start + MAX_UPLOAD_ROWS]


def output_path(out_dir, lang, part_idx, part_total, manual_only=False):
    if manual_only:
        stem = f"{lang}_缺失待平台翻译_{RUN_DATE}"
    else:
        stem = f"{lang}_补翻_{RUN_DATE}"
    if part_total > 1:
        stem = f"{stem}_{part_idx:02d}"
    return out_dir / f"{stem}.xlsx"


def write_workbook(lang, rows, path):
    ...
    for row_idx, row in enumerate(rows, 3):
        values = [row["keyId"], row["en"], row["translation"], row["remark"]]
        for col_idx, value in enumerate(values, 1):
            cell = ws.cell(row_idx, col_idx)
            set_text(cell, value)
            cell.alignment = Alignment(wrap_text=True, vertical="top")
            if row["remark"] and col_idx in (3, 4):
                cell.fill = RISK_FILL
...
def build_workbooks(lang, tasks, done):
    stats = {
        "lang": lang,
        "rows": 0,
        "ok_rows": 0,
        "risk_rows": 0,
        "manual_translation_rows": 0,
    ...
    for task in tasks:
        ...
        if lang in SKIP_TRANSLATE_LANGS:
            translation = ""
            issues = []
            stats["manual_translation_rows"] += 1
        ...
        all_rows.append(row)
        if not issues and lang not in SKIP_TRANSLATE_LANGS:
            uploadable_rows.append(row)
        stats["rows"] += 1
        if key in seen_keys:
    ...
    stats["uploadable_rows"] = len(uploadable_rows)
    stats["skipped_uploadable_rows"] = stats["rows"] - stats["uploadable_rows"]

    all_part_total = (len(all_rows) + MAX_UPLOAD_ROWS - 1) // MAX_UPLOAD_ROWS
    for part_idx, chunk in chunked(all_rows):
        path = output_path(OUTPUT_DIR, lang, part_idx, all_part_total, lang in SKIP_TRANSLATE_LANGS)
        stats["outputs"].append(write_workbook(lang, chunk, path))

    if uploadable_rows:
        uploadable_part_total = (len(uploadable_rows) + MAX_UPLOAD_ROWS - 1) // MAX_UPLOAD_ROWS
        for part_idx, chunk in chunked(uploadable_rows):
            path = output_path(UPLOADABLE_DIR, lang, part_idx, uploadable_part_total)
            stats["uploadable_outputs"].append(write_workbook(lang, chunk, path))
...
def main():
    ...
    parser.add_argument("--dify-only", action="store_true")
    ...
    done = hydrate_done_from_legacy_jsonl(tasks_by_lang, done)
    ...
    for lang, tasks in tasks_by_lang.items():
        stats = build_workbooks(lang, tasks, done)
        report["langs"][lang] = stats
        log(
            f"已生成 {lang}: rows={stats['rows']} ok={stats['ok_rows']} "
            f"risk={stats['risk_rows']} uploadable={stats['uploadable_rows']} "
            f"files={len(stats['outputs'])} uploadable_files={len(stats['uploadable_outputs'])}"
        )
    ...
    print(json.dumps({
        ...
        "langs": {
            lang: {
                ...
                "manual_translation_rows": stats["manual_translation_rows"],
                "uploadable_rows": stats["uploadable_rows"],
                "outputs": stats["outputs"],
                "uploadable_outputs": stats["uploadable_outputs"],
            }
